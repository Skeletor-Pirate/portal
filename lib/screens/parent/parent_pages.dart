...
class ParentPages extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    switch (page) {
      ...
      case 'childoverview': return _ChildOverview();
      case 'grades':        return _Grades();
      case 'attendance':    return _Attendance();
      case 'assignments':   return _Assignments();
      case 'payments':      return _Payments();
      case 'insights':      return _Insights();
...
class _Dashboard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Column(
    ...
    children: [
      ...
      actionGrid([
        ActionItem(icon: '📊', label: "Child's Grades",  bg: AppColors.blueLight),
        ActionItem(icon: '✅', label: 'Attendance',       bg: AppColors.greenLight),
        ActionItem(icon: '📋', label: 'Assignments',      bg: AppColors.tealLight),
        ActionItem(icon: '💳', label: 'Pay Fees',         bg: AppColors.redLight),
        ActionItem(icon: '✦',  label: 'AI Insights',      bg: AppColors.amberLight),
        ActionItem(icon: '💬', label: 'Message Teacher',  bg: AppColors.blueLight),
      ]),
...
class _ChildOverview extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Column(
    ...
    children: [
      ...
      statGrid([
        const StatItem(icon: '📊', val: '88%', label: 'Avg Grade',     delta: 3),
        const StatItem(icon: '✅', val: '91%', label: 'Attendance',    delta: 1),
        const StatItem(icon: '📋', val: '2',   label: 'Pending Tasks', delta: 0),
        const StatItem(icon: '🏆', val: 'B+',  label: 'GPA Band',      delta: 0),
      ]),
        ...
        Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Padding(
            ...
            child: Row(
              ...
              children: [
                Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text('83.5%', style: GoogleFonts.dmSerifDisplay(
                      fontSize: 26, color: AppColors.text1)),
                  Text("Arjun's Average · Term 2",
                      style: GoogleFonts.plusJakartaSans(fontSize: 11, color: AppColors.text3)),
                ]),
                Container(
                  padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 6),
                  decoration: BoxDecoration(
                      color: AppColors.avNavy, borderRadius: BorderRadius.circular(rFull)),
      ...
      appCard(
        Padding(
          ...
          child: Column(children: [
            ...
            Text('⚠️  Minimum required: 75% · Arjun is on track',
                style: GoogleFonts.plusJakartaSans(fontSize: 11, color: AppColors.text3)),
          ]),
        ),
      ),
...
class _Assignments extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Column(
    ...
    children: [
      ...
      appCard(asgnCards([
        AsgItem(sub: 'MATHEMATICS', title: 'Quadratic Equations Set B',
            due: '📅 Tomorrow', barColor: AppColors.blue,
            badge: 'Pending', badgeBg: AppColors.amberLight, badgeColor: AppColors.amber),
        AsgItem(sub: 'PHYSICS',     title: 'Chapter 4 Problems',
            due: '📅 Mar 30',   barColor: AppColors.teal,
            badge: 'Submitted', badgeBg: AppColors.blueLight, badgeColor: AppColors.blue),
        AsgItem(sub: 'HISTORY',     title: 'WWII Analysis Essay',
            due: '📅 Mar 25',   barColor: AppColors.green,
            badge: 'Graded · A', badgeBg: AppColors.greenLight, badgeColor: AppColors.green),
      ])),
      const SizedBox(height: 16),
    ],
  );
  ...
  Widget build(BuildContext context) => Column(
    ...
    children: [
      ...
      Padding(
        ...
        child: navyBtn('💳  Pay Now Online'),
      ),
      secLabel('Payment History'),
      appCard(invRows([
        InvItem(id: 'INV-089', name: 'Term 2 Tuition',  type: 'Apr 2025',
            amount: '₹24,500', status: 'Due',
            badgeBg: AppColors.amberLight, badgeColor: AppColors.amber),
        InvItem(id: 'INV-075', name: 'Term 1 Tuition',  type: 'Nov 2024',
            amount: '₹24,500', status: 'Paid',
            badgeBg: AppColors.greenLight, badgeColor: AppColors.green),
        InvItem(id: 'INV-062', name: 'Activity Fee',    type: 'Nov 2024',
            amount: '₹3,200',  status: 'Paid',
            badgeBg: AppColors.greenLight, badgeColor: AppColors.green),
        InvItem(id: 'INV-051', name: 'Examination Fee', type: 'Mar 2025',
            amount: '₹1,800',  status: 'Paid',
            badgeBg: AppColors.greenLight, badgeColor: AppColors.green),
      ])),
      const SizedBox(height: 16),
    ],
...
class _Insights extends StatelessWidget {
    ...
    children: [
      ...
      appCard(
        Padding(
          padding: const EdgeInsets.all(14),
          child: Column(children: [
            _insight(AppColors.blue,  '✦ Performance Trend',
                "Arjun's Physics scores have improved by 8% over the last 3 tests. Consistent practice is paying off."),
            const SizedBox(height: 14),
            _insight(AppColors.amber, '⚠️ Attendance Warning',
                '3 absences this month. Ensure regular attendance to maintain the 75% minimum threshold.'),
            const SizedBox(height: 14),
            _insight(AppColors.green, '📈 Predicted Grade',
                'Based on current performance, final grade predicted at B+ to A− range for Term 2.'),
          ]),
        ),
      ),
      const SizedBox(height: 16),
  ...
  Widget _insight(Color color, String title, String body) => Row(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      Container(width: 3, height: 55, color: color,
